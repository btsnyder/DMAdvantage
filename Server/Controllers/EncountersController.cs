    ...
    public class EncountersController : BaseEntityController<Encounter>
    {
        ...
        public async Task<IActionResult> CreateNewEncounter([FromBody] Encounter request)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (_context.SaveAll())
                    {
                        return Created($"/api/{GenericHelpers.GetPath<Encounter>()}/{entity.Id}", entity);
                    }
                }
                else
                {
        ...
        public async Task<IActionResult> UpdateEncounterById(Guid id, [FromBody] Encounter request)
        {
            try
            {
                ...
                _context.SaveAll();
                _producer?.SendMessage(new KafkaMessage { Topic = Topics.Encounters, User = username, Value = KafkaValues.Updated });
                return NoContent();
            }
            catch (Exception ex)
            {
